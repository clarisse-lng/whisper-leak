...
class ModelTrainer:
    ...
    """
    def __init__(self, model, config, device, n_classes=2):
        """
        ...
        if isinstance(model, nn.Module):
            self.criterion = nn.CrossEntropyLoss() if n_classes > 2 else nn.BCEWithLogitsLoss()
        else:
            self.criterion = None
    ...
    def _train_pytorch_model(self, train_loader, val_loader, optimizer):
        ...
        for epoch in range(self.config.max_epochs):
            ...
            for batch_idx, (X, y) in enumerate(train_loader):
                ...
                if self.n_classes > 2:
                    y_dev = y.to(self.device).long()
                    loss = self.criterion(outputs, y_dev)
                    with torch.no_grad():
                        preds = torch.argmax(outputs, dim=1)
                        train_acc += (preds == y_dev).float().mean().item()
                else:
                    y_dev = y.to(self.device).float().unsqueeze(1)
                    loss = self.criterion(outputs, y_dev)
                    with torch.no_grad():
                        preds = (torch.sigmoid(outputs) > 0.5).float()
                        train_acc += (preds == y_dev).sum().item() / y_dev.size(0)
            ...
            with torch.no_grad():
                for X, y in val_loader:
                    ...
                    if self.n_classes > 2:
                        ...
                        val_loss += loss.item()
                        preds = torch.argmax(outputs, dim=1)
                        val_acc += (preds == y_dev).float().mean().item()
                    else:
                        y_dev = y.to(self.device).float().unsqueeze(1)
                        loss = self.criterion(outputs, y_dev)
                        val_loss += loss.item()
                        preds = (torch.sigmoid(outputs) > 0.5).float()
                        val_acc += (preds == y_dev).sum().item() / y_dev.size(0)
    ...
    def predict(self, data, batch_size=32):
        ...
        if isinstance(self.model, LightGBMClassifier):
            ...
            with torch.no_grad():
                for X, y in data:
                    ...
                    if self.n_classes > 2:
                        ...
                        scores = torch.softmax(outputs, dim=1).cpu().numpy()  # [batch, n_classes]
            ...
            if self.n_classes > 2:
                all_scores = np.vstack(all_scores)  # [n_samples, n_classes]
            else:
            ...
            scores = self.model.predict_scores(data.df)
...
def load_chatbot_data(chatbot_name, input_folder, prompts_file, downsample_rate=1.0):
    ...
    # Use target from data if already set, otherwise derive from prompts file
    ...
    if 'target' not in df.columns or df['target'].isnull().all():
        ...
        positive_prompts = set(prompts['positive']['prompts'])
        df['target'] = df.apply(
            lambda row: (1 if row['prompt'] in positive_prompts else 0) if pd.isnull(row['target']) else row['target'],
            axis=1
        )
