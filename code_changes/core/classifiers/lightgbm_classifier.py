...
class LightGBMClassifier:
    ...
    """
    def __init__(self, norm, n_classes=2, **kwargs):
        """
        ...
        Args:
            ...
            n_classes (int): Number of output classes (2 for binary, >2 for multiclass).
    ...
    def _prepare_features(self, df):
        ...
        cols = [f'time_{i}' for i in range(self.max_len)] + [f'len_{i}' for i in range(self.max_len)]
        return pd.DataFrame(X, columns=cols)

    def fit(self, train_df, val_df, patience=40, **kwargs):
        """
    ...
    def predict_proba(self, df):
        ...
        Returns:
            np.ndarray: Shape (n_samples,) for binary or (n_samples, n_classes) for multiclass.
        """
        if not self.has_fit():
    ...
    def inference(self, input_data, device=None):
        if isinstance(input_data, pd.DataFrame):
            scores = self.predict_proba(input_data)
    ...
    def load(cls, filepath, device=None): # device is ignored for LGBM but kept for compatibility
        ...
        n_classes = meta_data.get('n_classes', 2)

        # Instantiate the class
        # Pass norm and stored args to the constructor
        instance = cls(norm=normalization_params, n_classes=n_classes, **args)
