...
class BaseClassifier(nn.Module):
    ...
    def load(cls, filepath, device):
        ...
        elif class_name == "RNNClassifier":
            module_name = "core.classifiers.rnn_classifier"
        else:
    ...
    def inference(self, input_data, device):
        ...
        if isinstance(input_data, pd.DataFrame):
            ...
            multiclass = hasattr(self, 'n_classes') and self.n_classes > 2
            all_probs = []
            with torch.no_grad():
                for X, _ in loader:
                    X = X.to(device)
                    output = self(X)
                    if multiclass:
                        probs = torch.softmax(output, dim=-1).cpu().numpy()  # [batch, n_classes]
        ...
        # Handle a 2-tuple
            ...
            with torch.no_grad():
                ...
                if hasattr(self, 'n_classes') and self.n_classes > 2:
                    probs = torch.softmax(output, dim=-1).cpu().numpy().flatten()  # [n_classes]
