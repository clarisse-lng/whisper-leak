...
class PrintUtils(object):
    ...
    def start_stage(cls, message, override_prev=False):
        ...
        # Cap line width to terminal width to prevent wrapping (which breaks \r override)
        term_width = shutil.get_terminal_size((cls.PP_LEN, 24)).columns
        line_width = min(cls.PP_LEN, max(40, term_width - 1))
        ...
        # Print title
        title = message[:line_width - 5]
        title += ' ...'
        title += (line_width - 1 - len(title)) * '.'
        title += ' '
