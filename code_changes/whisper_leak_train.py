...
def parse_arguments():
    ...
    parser.add_argument('--results-dir', type=str, help='Results staging directory name (default: results)', default='results')
...
def create_model(model_type, norm, kernel_width=3, df_train=None, n_classes=2):
    """
    ...
    suffix = 'binary' if n_classes == 2 else f'multiclass{n_classes}'
    model_type = model_type.upper()
    if model_type == 'CNN':
        model = CNNClassifier(norm, kernel_width)
        model_path = os.path.join(models_dir, f'cnn_{suffix}_classifier.pth')
    elif model_type == 'LSTM':
        model = AttentionBiLSTMClassifier(norm)
        model_path = os.path.join(models_dir, f'lstm_{suffix}_classifier.pth')
    elif model_type == 'LSTM_BERT':
        model = CombinedLSTMBERTClassifier(norm)
        model_path = os.path.join(models_dir, f'lstm_bert_{suffix}_classifier.pth')
    elif model_type == 'KNN':
        model = KNNClassifier(norm, n_neighbors=15, n_classes=n_classes)
        model_path = os.path.join(models_dir, f'knn_{suffix}_classifier.pth')
    elif model_type == 'LGBM':
        model = LightGBMClassifier(norm, n_classes=n_classes)
        model_path = os.path.join(models_dir, f'lightgbm_{suffix}_classifier.pth')
    elif model_type == 'PERCEPTRON':
        model = PerceptronClassifier(max_len=norm['max_len'], n_classes=n_classes)
        model_path = os.path.join(models_dir, f'perceptron_{suffix}_classifier.pth')
    elif model_type == 'RNN':
        model = RNNClassifier(norm, n_classes=n_classes)
        model_path = os.path.join(models_dir, f'rnn_{suffix}_classifier.pth')
        ...
        model_path = os.path.join(models_dir, f'bert_{suffix}_classifier.pth')
    else:
...
def main():
    ...
    try:
        ...
        PrintUtils.print_extra(f'Number of classes detected: *{n_classes}*')
        ...
        model_trainer = ModelTrainer(model, config, device, n_classes=n_classes)
        ...
        if n_classes > 2:
            for cls_idx in range(n_classes):
                df_test_results[f'score_class_{cls_idx}'] = test_scores[:, cls_idx]
        else:
        ...
        prob_str = np.array2string(np.array(prob), precision=4) if n_classes > 2 else f'{prob:.4f}'
        PrintUtils.print_extra(f'Inference result (tuple input): prob=*{prob_str}*, pred=*{pred}*')

        # Test with DataFrame input
        sample_df = df_test.iloc[[0]]
        probs, preds = loaded_model.inference(sample_df, device)
        prob0_str = np.array2string(np.array(probs[0]), precision=4) if n_classes > 2 else f'{probs[0]:.4f}'
        PrintUtils.print_extra(f'Inference result (DataFrame input): prob=*{prob0_str}*, pred=*{preds[0]}*')
        PrintUtils.end_stage()
