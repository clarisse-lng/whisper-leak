...
class ModelTrainer:
    ...
    def _train_pytorch_model(self, train_loader, val_loader, optimizer):
        ...
        for epoch in range(self.config.max_epochs):
            ...
            # AVANT — dans la boucle train et val, partout :
			...
			# APRES
                if self.num_classes == 2:
					...
					outputs = self.model(X)          # shape: (B, num_classes)
					loss = self.criterion(outputs, y)
            ...
            with torch.no_grad():
                for X, y in val_loader:
                    ...
                    preds = (torch.sigmoid(outputs) > 0.5).float()
                    PrintUtils.print_extra(f"logits mean: {outputs.mean().item():.4f}")
...
def load_chatbot_data(chatbot_name, input_folder, prompts_file, downsample_rate=1.0):
	...
	# AVANT (fin de la fonction)
	# df['target'] = df['prompt'].apply(lambda x: 1 if x in prompts['positive']['prompts'] else 0)

	# APRÈS — garder ce comportement uniquement pour le format legacy
	# La fonction load_multitheme_data dans whisper_leak_train.py écrase 'target' après coup,
	# mais load_chatbot_data est appelée avec le nouveau JSON multithème, dont la structure
	# n'a pas les clés 'positive'/'negative' → crash garanti.
	# Solution : rendre l'assignation de target conditionnelle :
	if 'positive' in prompts and 'negative' in prompts:
		...
		# Format multithème : target sera assigné par load_multitheme_data
		df['target'] = -1  # valeur sentinelle, sera écrasée
