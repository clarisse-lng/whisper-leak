        ...
        for (prompt, pertubated_prompt, index) in task_list:
            ...
            for attempt in range(3):
                ...
                try:
                    ...
                    if attempt < 2:
                        PrintUtils.print_extra(f'Retry {attempt + 2}/3 [{type(e).__name__}]: {str(e)}')
                        time.sleep(random.uniform(15, 30))
                    else:
                        PrintUtils.print_extra(f'Exception [{type(e).__name__}]: {str(e)}')
                        PrintUtils.print_extra(traceback.format_exc())
                        failed += 1
